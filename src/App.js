...
const teams = {
  ...
  japan: { code: 'JPN', color: '#e30613', trim: '#ffffff', text: '#ffffff', labels: { en: 'Team Japan', nl: 'Team Japan', ja: 'チーム・日本' } },
};

const keeperKits = {
  netherlands: { primary: '#f47b20', trim: '#ffffff', text: '#101114' },
  japan: { primary: '#174a9c', trim: '#e30613', text: '#ffffff' },
};
...
const copy = {
  en: {
    round: 'Round', progress: 'Progress', answer: 'Choose your answer', timeout: 'TIME OUT!', goal: 'GOAL!', save: 'SAVED!', next: 'Next round', finish: 'Finish game', complete: 'Finished', playAgain: 'Play again', changeSetup: 'Change setup',
    formsTitle: 'Join the prize draw', formsText: 'Open Microsoft Forms to submit your entry. Winners are selected at random.', formsButton: 'Open prize form', points: 'points', correct: 'correct', leaderboard: 'Leaderboard', noScores: 'No scores yet.',
  ...
  nl: {
    round: 'Ronde', progress: 'Voortgang', answer: 'Kies je antwoord', timeout: 'TIJD OM!', goal: 'GOAL!', save: 'GEPAKT!', next: 'Volgende ronde', finish: 'Game afronden', complete: 'Klaar', playAgain: 'Speel opnieuw', changeSetup: 'Setup wijzigen',
    formsTitle: 'Doe mee aan de winactie', formsText: 'Open Microsoft Forms om je deelname in te sturen. De winnaar wordt willekeurig gekozen.', formsButton: 'Open winactie formulier', points: 'punten', correct: 'goed', leaderboard: 'Leaderboard', noScores: 'Nog geen scores.',
  ...
  ja: {
    round: 'ラウンド', progress: '進行状況', answer: '答えを選択', timeout: '時間切れ！', goal: 'ゴール！', save: 'セーブ！', next: '次のラウンド', finish: 'ゲーム終了', complete: '終了', playAgain: 'もう一度プレイ', changeSetup: '設定変更',
    formsTitle: '抽選に参加', formsText: 'Microsoft Formsを開いて参加してください。当選者はランダムに選ばれます。', formsButton: 'フォームを開く', points: 'ポイント', correct: '正解', leaderboard: 'リーダーボード', noScores: 'まだスコアがありません。',
        ...
        <div className='setup-panel'>
          ...
          <div className='setup-group'>
            ...
            <div className='segmented-control' aria-label={setupCopy.language}>
              {languages.map((option) => <button className={cx(language === option.id && 'active')} key={option.id} type='button' onClick={() => setLanguage(option.id)}>{option.label}</button>)}
            </div>
          </div>
    ...
    <main className='app app-game'>
      <section className='game-shell'>
        ...
        <div className='game-grid'>
          <section className={cx('question-card', questionData.visualOnly && 'visual-only-question', questionData.imageChoices && 'image-choice-question', questionData.swatches && 'swatch-question', questionData.plain && 'plain-question')}>
            ...
            <div className='answer-grid'>
              {localizedQuestion.answers.map((answer, index) => {
                ...
                return (
                  <button className={cx('answer-button', !visualType && 'no-visual-answer', answerClass(index))} disabled={answered} key={`${questionData.id}-${answer}`} type='button' onClick={() => handleAnswer(index)} aria-label={answer}>
          ...
          <PenaltyArena outcome={outcome} animationId={animationId} playerTeam={match.playerTeam} keeperKit={match.keeperKit} />
        </div>
      </section>
    </main>
  );
